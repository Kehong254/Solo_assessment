...
def store(request):
    ...
    if query:
        volcanoes = volcanoes.filter(
            Q(Volcano_Name__icontains=query) | Q(Country__icontains=query) | Q(Volcano_Type__icontains=query)).order_by(
            '-Volcano_ID')
...
#             return render(request, 'cart.html', {'cart_list': cart_list, 'new_product': {'Volcano_Name': Volcano_Name, 'price': price}, 'total': total})
#         else:
#             cart_list = cart.objects.all()
#             total = sum([item.price * item.quantity for item in cart_list])
#             return render(request, 'store/cart.html', {'cart_list': cart_list, 'total': total})
